    ...
    function fetchPlantCareTips() {
        if (devMode) {
            ...
            fetch("/api/proxy/generateContent", { // Update to use Vercel proxy
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    contents: [{ parts: [{ text: "Get plant care tips." }] }]
                }),
            })
            .then(response => {
                if (!response.ok) {
    ...
    function updateHumidity() {
      ...
      if (devMode) {
        ...
        timestamps.push(currentTimeString);
        filterOldData(currentTime);
        updateChart(false);
      } else {
    ...
    function filterOldData(currentTime) {
      const currentTimeMs = currentTime.getTime();
      
      // Keep only data from the last 30 seconds
      timestamps.forEach((timestamp, index) => {
        const timestampMs = new Date(`1970-01-01T${timestamp}`).getTime();
        if (currentTimeMs - timestampMs > MAX_DURATION) {
          timestamps.splice(index, 1);
          humidityData.forEach(data => data.splice(index, 1));
        }
      });
    }
